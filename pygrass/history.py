...
class History(object):
    ...
    def read(self, name):
        """Rast_read_history
        hist_ok = Rast_read_history(name, "", &hist)
        >>> import grass.lib.gis as libgis
        >>> libgis.G_gisinit('')
        >>> import ctypes
        >>> import grass.lib.raster as libraster
        >>> hist = libraster.History()
        >>> libraster.Rast_read_history(ctypes.c_char_p('aspect'), ctypes.c_char_p(''), ctypes.byref(hist))
        >>> libraster.Rast_get_history(ctypes.byref(hist), libraster.HIST_MAPID)
        'Tue Nov  7 01:11:23 2006'
        """
